...
def handle_relu(model,var_list, affine_counter, num_neurons, lbi, ubi, relu_groupsi, use_milp, milp_for_fc=False):
    use_milp = use_milp and config.use_milp
    ...
    for i in range(numlayer):
        ...
        if(nn.layertypes[i] in ['SkipCat']):
            ...
            #    counter = handle_relu(model,var_list, counter,len(nlb[i]),nlb[index-1],nub[index-1], relu_groups[nn.activation_counter], use_milp)
                ...
                counter = handle_relu(model,var_list, counter,len(nlb[i]),nlb[index-1],nub[index-1], relu_groups[nn.activation_counter], use_milp, milp_for_fc)
